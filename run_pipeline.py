...
# Utility for formatted console logs
def log_step(title):
    print(f"\n============================================================")
    print(f" 🚀 {title}")
    print(f"============================================================")
...
def check_step_a():
    """Step A Check: Raw JSON File Download & Decompression"""
    if not RAW_JSON_PATH.exists():
        log_status("Substep A.1: Raw AllPrices.json existence", False)
        return False
    
    # Substep A.2: Check file size (> 100MB ensures complete extraction)
    ...
    log_status("Substep A.1: Raw AllPrices.json existence", True)
    log_status("Substep A.2: File size verification (>100MB)", True, f"{file_size_mb:.1f} MB")
    return True


def check_step_b():
    """Step B Check: DuckDB Ingestion & Raw Table Loading"""
    ...
    try:
        ...
        if "fact_prices" not in tables:
            log_status("Substep B.2: 'fact_prices' table existence", False)
            conn.close()
            return False
        ...
        if count == 0:
            log_status("Substep B.3: 'fact_prices' row count (>0)", False)
            return False
            
        log_status("Substep B.1: DuckDB database file existence", True)
        log_status("Substep B.2: 'fact_prices' table existence", True)
        log_status("Substep B.3: 'fact_prices' row count", True, f"{count:,} rows")
        return True
...
def main():
    ...
    # ---------------------------------------------------------
    # STEP A: Download & Extract Raw Payload
    # ---------------------------------------------------------
    ...
    if args.force or not check_step_a():
        ...
        print("  ⏩ Skipping Step A — Raw data payload is already downloaded & extracted.")

    # ---------------------------------------------------------
    # STEP B: DuckDB Ingestion
    # ---------------------------------------------------------
    ...
    if args.force or not check_step_b():
        ...
        print("  ⏩ Skipping Step B — DuckDB 'fact_prices' table is already populated.")

    # ---------------------------------------------------------
    # STEP C: Feature Engineering & Arbitrage Views
    # ---------------------------------------------------------
